...
(
    id             bigserial PRIMARY KEY,
    customer_login varchar(255) NOT NULL REFERENCES tires.customers (login) ON DELETE SET NULL,
    creation_time timestamp with time zone,
    deleted boolean default false
);

CREATE TABLE IF NOT EXISTS tires.order_option (
    order_id bigserial NOT NULL REFERENCES tires.orders (id) ON DELETE SET NULL,
    option_id bigserial NOT NULL REFERENCES tires.options (id) ON DELETE SET NULL
);